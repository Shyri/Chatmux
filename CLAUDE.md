...
- **The chat transcript has its own lazy-layout guard.** `scripts/check-chat-lazy-layout.py` (sibling of `check-sidebar-lazy-layout.py`, meta-tested by `tests/test_ci_chat_lazy_layout_guard.py` in `workflow-guard-tests`) pins the contract for `chatContent` / `chatColumn` / `messageList`, the transcript row views, and `ChatDropContainer`. Three production livelocks in one week fed it, each found only by sampling a hung app: `ChatDropContainer` without `sizeThatFits` let the nested `NSHostingView` measure the whole `LazyVStack` (17% of the main thread, `a9712e8b90`); a `GeometryReader` wrapping the chat to read one width mediated every measurement below it (17%, `1746ae85c0`); and that same hosting view, though pinned to four edges, kept an `intrinsicContentSize` in sync with its content, so each update posted `setNeedsUpdateConstraints` to the window and the resulting display cycle re-fired the lazy list's prefetch (**55%**, `0732d68bc5`). `sizeThatFits` and `sizingOptions = []` are one-liners that look removable and are not — the guard exists because no test reproduces their absence: a scale test mounting the panel standalone cannot recreate the SwiftUI-in-AppKit-in-SwiftUI sandwich that produces the loop.
